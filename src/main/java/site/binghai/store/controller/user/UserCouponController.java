...
public class UserCouponController extends BaseController {
    ...
    public String couponList(Integer exp, Long unifiedId, ModelMap map) {
        ...
        couponList = couponList.stream()
                ...
                .filter(v -> order == null || v.getAppCode().equals(order.getAppCode()) || v.getAppCode().equals(PayBizEnum.ALL.getCode()))
    ...
    public String access2Coupon(@RequestParam String uuid, ModelMap map) {
        ...
        serverPushCoupon(ticket, getUser());
     ...
     * 系统内部发券
     */
    public CouponTicket serverPushCoupon(CouponTicket ticket, User toWho) {
        ...
        coupon.setOpenId(toWho.getOpenId());
        coupon.setUserId(toWho.getId());
        coupon.setRegionId(ticket.getRegionId());
        coupon.setUserName(toWho.getUserName());
